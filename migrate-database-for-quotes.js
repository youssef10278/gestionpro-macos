 ...
 * Basé sur la structure existante des ventes et bons de livraison
 */
...
try {
    ...
    // Migration 4: Insérer des données de test (optionnel)
    console.log('➕ Insertion de données de test...');
    
    // Vérifier s'il y a des clients et utilisateurs
    const clientCount = db.prepare('SELECT COUNT(*) as count FROM clients').get().count;
    const userCount = db.prepare('SELECT COUNT(*) as count FROM users').get().count;
    
    if (clientCount > 0 && userCount > 0) {
        const firstClient = db.prepare('SELECT * FROM clients LIMIT 1').get();
        ...
        console.log('⚠️ Pas de clients ou utilisateurs, pas de données de test créées');
    }
