...
class TemplateDesigner {
    ...
    initializeElements() {
        ...
        // Bouton de réinitialisation des marges
        this.resetMarginsBtn = document.getElementById('resetMarginsBtn');
        ...
        // Bouton de réinitialisation du pied de page
        this.resetFooterBtn = document.getElementById('resetFooterBtn');
    }
    ...
    bindEvents() {
        ...
        // Sliders pour les marges (en mm)
        this.bindSlider(this.layoutInputs.marginTop, this.layoutInputs.marginTopValue, 'mm');
        this.bindSlider(this.layoutInputs.marginRight, this.layoutInputs.marginRightValue, 'mm');
        this.bindSlider(this.layoutInputs.marginBottom, this.layoutInputs.marginBottomValue, 'mm');
        this.bindSlider(this.layoutInputs.marginLeft, this.layoutInputs.marginLeftValue, 'mm');
        ...
        // Bouton de réinitialisation des marges
        if (this.resetMarginsBtn) {
            this.resetMarginsBtn.addEventListener('click', () => {
                this.resetMargins();
            });
        }

        // Événements pour les champs du pied de page
        Object.values(this.footerInputs).forEach(input => {
            if (input) {
        ...
        // Bouton de réinitialisation du pied de page
        if (this.resetFooterBtn) {
            this.resetFooterBtn.addEventListener('click', () => {
                this.resetFooter();
            });
        }
    }
    ...
    loadTemplateIntoForm(template) {
        ...
        // Charger les marges de la page
        const pageMargins = layout.page_margins || {};
        const marginTopParsed = parseInt(pageMargins.top) || 20;
        const marginRightParsed = parseInt(pageMargins.right) || 20;
        const marginBottomParsed = parseInt(pageMargins.bottom) || 20;
        const marginLeftParsed = parseInt(pageMargins.left) || 20;

        console.log('  - Marges parsées:', {
            top: marginTopParsed,
            right: marginRightParsed,
            bottom: marginBottomParsed,
            left: marginLeftParsed
        });

        this.setSliderValueSilent('marginTop', marginTopParsed, 'mm');
        this.setSliderValueSilent('marginRight', marginRightParsed, 'mm');
        this.setSliderValueSilent('marginBottom', marginBottomParsed, 'mm');
        this.setSliderValueSilent('marginLeft', marginLeftParsed, 'mm');
        ...
        try {
            if (template.footer_content) {
                if (typeof template.footer_content === 'string') {
                    footerContent = JSON.parse(template.footer_content);
                } else {
                    footerContent = template.footer_content;
                }
            }
        } catch (error) {
            console.warn('⚠️ Erreur lors du parsing du footer_content:', error);
            footerContent = {};
        }

        console.log('📄 Chargement du contenu du pied de page:', footerContent);
        console.log('📄 Type de footer_content original:', typeof template.footer_content);
        console.log('📄 Valeur de footer_content original:', template.footer_content);

        if (this.footerInputs.thankYou) {
            const value = footerContent.thank_you_message || 'Merci pour votre confiance';
            this.footerInputs.thankYou.value = value;
            console.log('📝 thankYou chargé:', value);
        }
        if (this.footerInputs.paymentTerms) {
            const value = footerContent.payment_terms || 'Conditions de paiement: 30 jours';
            this.footerInputs.paymentTerms.value = value;
            console.log('📝 paymentTerms chargé:', value);
        }
        if (this.footerInputs.customMessage) {
            const value = footerContent.custom_message || 'Cette facture est générée automatiquement par le système de gestion.';
            this.footerInputs.customMessage.value = value;
            console.log('📝 customMessage chargé:', value);
        }
        if (this.footerInputs.legalInfo) {
            const value = footerContent.legal_info || 'ICE: 123456789012345 • RC: 12345 • CNSS: 67890';
            this.footerInputs.legalInfo.value = value;
            console.log('📝 legalInfo chargé:', value);
        }
    ...
    getTemplateDataFromForm() {
        ...
        let marginBottomValue = '20';
        let marginLeftValue = '20';
        ...
        // Récupération des valeurs des marges
        if (this.layoutInputs.marginTop && this.layoutInputs.marginTop.value !== undefined) {
            marginTopValue = this.layoutInputs.marginTop.value;
        }
        if (this.layoutInputs.marginRight && this.layoutInputs.marginRight.value !== undefined) {
            marginRightValue = this.layoutInputs.marginRight.value;
        }
        if (this.layoutInputs.marginBottom && this.layoutInputs.marginBottom.value !== undefined) {
            marginBottomValue = this.layoutInputs.marginBottom.value;
        }
        if (this.layoutInputs.marginLeft && this.layoutInputs.marginLeft.value !== undefined) {
            marginLeftValue = this.layoutInputs.marginLeft.value;
        }
        ...
        const templateData = {
            ...
            layout_config: {
                ...
                page_margins: {
                    top: marginTopValue + 'mm',
                    right: marginRightValue + 'mm',
                    bottom: marginBottomValue + 'mm',
                    left: marginLeftValue + 'mm'
                }
            },
            elements_config: {
        ...
        // Debug spécifique pour le footer_content
        console.log('💾 DEBUG - Sauvegarde footer_content:');
        console.log('   - thankYou input value:', this.footerInputs.thankYou?.value);
        console.log('   - paymentTerms input value:', this.footerInputs.paymentTerms?.value);
        console.log('   - customMessage input value:', this.footerInputs.customMessage?.value);
        console.log('   - legalInfo input value:', this.footerInputs.legalInfo?.value);
        console.log('   - footer_content final:', templateData.footer_content);
    ...
    generatePreviewCSS(templateData) {
        ...
        return `
            ...
            /* Styles d'impression avec marges */
            @media print {
                @page {
    ...
    generatePreviewHTML(templateData) {
        ...
        return `
            <div class="preview-invoice">
                ...
                ${elements.show_legal_mentions ? this.generateFooterHTML(templateData.footer_content) : ''}
            </div>
        `;
    }

    /**
     * Génère le HTML du pied de page pour l'aperçu
     */
    generateFooterHTML(footerContent) {
        if (!footerContent) {
            return '';
        }
        ...
        if (footer.thank_you_message && footer.thank_you_message.trim()) {
            footerHTML += `
                <div class="text-center mb-3 font-medium text-blue-600">
        ...
        if (footer.payment_terms && footer.payment_terms.trim()) {
            footerHTML += `
        ...
        if (footer.custom_message && footer.custom_message.trim()) {
            footerHTML += `
                <div class="text-center mb-3 italic">
                    ${footer.custom_message}
                </div>
            `;
        }

        // Informations légales
        if (footer.legal_info && footer.legal_info.trim()) {
            footerHTML += `
    ...
    /**
     * Réinitialiser les marges aux valeurs par défaut
     */
    resetMargins() {
        console.log('🔄 Réinitialisation des marges...');
        ...
        this.setSliderValueSilent('marginTop', defaultMargins.top, 'mm');
        this.setSliderValueSilent('marginRight', defaultMargins.right, 'mm');
        this.setSliderValueSilent('marginBottom', defaultMargins.bottom, 'mm');
        this.setSliderValueSilent('marginLeft', defaultMargins.left, 'mm');

        // Mettre à jour l'aperçu
        this.updatePreview();
        this.markDirty();

        console.log('✅ Marges réinitialisées aux valeurs par défaut');
    }

    /**
     * Réinitialise le contenu du pied de page aux valeurs par défaut
     */
    resetFooter() {
        console.log('🔄 Réinitialisation du pied de page...');
        ...
        // Appliquer les valeurs par défaut
        Object.keys(defaultFooter).forEach(key => {
            if (this.footerInputs[key]) {
                this.footerInputs[key].value = defaultFooter[key];
            }
        });
        ...
        console.log('✅ Pied de page réinitialisé aux valeurs par défaut');
    }
