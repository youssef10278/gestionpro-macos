...
function initDatabase() {
    db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS invoice_templates (
            ...
            elements_config TEXT NOT NULL DEFAULT '{}',
    ...
    // Migration : Ajouter les colonnes manquantes aux templates de factures
    try {
        console.log('🔍 Vérification des colonnes de templates...');
        const templateColumns = db.prepare("PRAGMA table_info(invoice_templates)").all();
        const hasElementsConfig = templateColumns.some(col => col.name === 'elements_config');
        const hasFooterContent = templateColumns.some(col => col.name === 'footer_content');

        console.log(`📊 État des colonnes templates: elements_config=${hasElementsConfig}, footer_content=${hasFooterContent}`);

        if (!hasElementsConfig) {
            console.log('Migration: Ajout de la colonne elements_config à la table invoice_templates');
            ...
            db.prepare("UPDATE invoice_templates SET elements_config = ? WHERE elements_config = '{}'").run(defaultElementsConfig);
            console.log('✅ Colonne elements_config ajoutée et initialisée');
        }

        if (!hasFooterContent) {
            console.log('Migration: Ajout de la colonne footer_content à la table invoice_templates');
            ...
            db.prepare("UPDATE invoice_templates SET footer_content = ? WHERE footer_content = '{}'").run(defaultFooterContent);
            console.log('✅ Colonne footer_content ajoutée et initialisée');
        }

        if (hasElementsConfig && hasFooterContent) {
            console.log('✅ Toutes les colonnes de templates sont présentes');
        }
    } catch (error) {
...
function createTemplate(templateData) {
    try {
        console.log('💾 DEBUG createTemplate - Données reçues:', templateData);
        ...
        const result = stmt.run(
            ...
            JSON.stringify(templateData.elements_config || {}),
            JSON.stringify(templateData.footer_content || {}),
            templateData.logo_path || null,
            templateData.user_created || 1
        );

        console.log('✅ Template créé avec succès, ID:', result.lastInsertRowid);
...
function updateTemplate(id, templateData) {
    try {
        console.log('💾 DEBUG updateTemplate - ID:', id, 'Données:', templateData);

        const stmt = db.prepare(`
            UPDATE invoice_templates
            SET display_name = ?, colors_config = ?, fonts_config = ?,
                layout_config = ?, elements_config = ?, footer_content = ?, logo_path = ?,
                updated_at = strftime('%Y-%m-%d %H:%M:%S', 'now', 'localtime')
        ...
        console.log('✅ Template mis à jour avec succès, changes:', result.changes);
